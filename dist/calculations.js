...
export function getDateInfo(config) {
    const now = new Date();
...
export function costForDate(days, date) {
    return days
...
function dayForDate(days, date) {
    return days.find((d) => d.date === date);
}
export function computeMetrics(config, days, dates) {
...
export function computeAlerts(config, metrics) {
    const alerts = [];
...
export function buildState(config, currentState, metrics, alertResult) {
    return {
        lastCheck: Date.now(),
