...
// ─── Currency Helpers ────────────────────────────────────────────
function toEur(usd, config) {
    return usd * config.usdToEur;
}
function formatEur(usd, config) {
    return `€${toEur(usd, config).toFixed(2)}`;
}
function formatEurBudget(usd, config) {
    return `€${toEur(usd, config).toFixed(0)}`;
}
function formatDual(usd, config) {
    return `$${usd.toFixed(2)} (€${toEur(usd, config).toFixed(2)})`;
}
function formatDualBudget(usd, config) {
    return `$${usd.toFixed(0)} (€${toEur(usd, config).toFixed(0)})`;
}
// ─── Text Formatter ──────────────────────────────────────────────
function formatText(state, config) {
    const lines = [];
...
function colorCostDual(costUsd, config) {
...
function colorProjectionDual(projUsd, config) {
    const str = formatDual(projUsd, config);
    if (projUsd > config.weeklyBudget)
        return chalk.red.bold(str + ' ⚠️');
    if (projUsd > config.weeklyBudget * 0.8)
        return chalk.yellow(str);
    return chalk.green(str);
}
// ─── JSON Formatter ──────────────────────────────────────────────
function formatJson(state, _config) {
    return JSON.stringify(state, null, 2);
}
// ─── HTML Formatter (for email briefings) ────────────────────────
function formatHtml(state, config) {
    const prot = state.protection_mode;
...
// ─── Dispatcher ──────────────────────────────────────────────────
export function formatReport(state, config, format) {
    switch (format) {
...
export function formatWeekSummary(state, config) {
    const lines = [];
    lines.push(chalk.bold('📊 Current Week'));
    lines.push(`  Period:     ${state.current_week.start_date} → ${state.current_week.end_date}`);
    lines.push(`  Total:      ${colorCostDual(state.current_week.total_cost, config)} / ${formatDualBudget(config.weeklyBudget, config)}`);
...
// ─── Daily Breakdown ─────────────────────────────────────────────
export function formatDailyBreakdown(state, config) {
    const lines = [];
...
// ─── Protection Status ───────────────────────────────────────────
export function formatProtectionStatus(status, config) {
    const lines = [];
...
export function formatSevenDaysDetail(state, config) {
    const lines = [];
